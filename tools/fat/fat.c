...
typedef struct {
    ...
    uint8_t MediaSubcriptorType;
    uint16_t SectorsPerFat;
    uint16_t SectorsPerTrack;
    uint16_t Heads;
    uint32_t HiddenSectors;
    uint32_t LargeSectorCount;
    uint8_t DriveNumbers;
    uint8_t _Reserved;
    uint8_t Signature;
    uint32_t VolumeId;
...
bool readFat(FILE* disk) {
    ...
    return readSectors(disk, g_BootSector.ReversedSectors, g_BootSector.SectorsPerFat, g_Fat);
}
